...
from bayesian_testing.metrics import eval_delta_exponential_agg
from bayesian_testing.utilities import get_logger

logger = get_logger("bayesian_testing")


class DeltaExponentialDataTest(BaseDataTest):
    """
    ...
    def evaluate(
        ...
        posterior_mean = [
            round((i[3] + i[1]) / (i[2] + i[0]) * ((i[4] + i[6]) / (i[5] + i[0])) , 5)
            for i in zip(self.totals, 
                         self.sum_values, 
                         self.a_priors, 
                         self.b_priors,
                         self.a_priors_beta,
    ...
    def add_variant_data(
        ...
        self.add_variant_data_agg(name, positives, totals, sum_values, 
                                  a_prior_beta,
                                  b_prior_beta,
                                  a_prior, b_prior, replace)
